...
struct Handler: APIProtocol {
    ...
    func createChant(_ input: Operations.CreateChant.Input) async throws -> Operations.CreateChant.Output {
        ...
        let client = ChatGPT.Client(
            serverURL: URL(string: "https://api.openrouter.com/v1")!,
            transport: AsyncHTTPClientTransport(),
            middlewares: [HeaderFieldMiddleware.authTokenFromEnvironment("OPENROUTER_TOKEN")]
        )
        ...
        let chatGPTResponse = try await client.createChatCompletion(
            body: .json(.init(messages: messages, model: .init(value2: .gpt-oss-20b:free), stream: true, temperature: 0))
        )
